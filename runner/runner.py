...
class Runner:
    ...
    def run_with_unittest(self):
        ...
        if enable_html:
            ...
            pj_cf = config.get('project', {})
            tester = pj_cf.get('executor', "自动触发")
    ...
    def run_mixed(self):
        ...
        if unittest_count > 0:
            ...
            if enable_html:
                ...
                xtestrunner_config = reports_config.get('xtestrunner', {})
                title = xtestrunner_config.get('title', "API Automated Test Report") + " - Unittest"
                description = xtestrunner_config.get('description', "Generated by XTestRunner")
                language = xtestrunner_config.get('language', "zh-CN")
                retry = xtestrunner_config.get('retry', 0)
                pj_cf = config.get('project', {})
        ...
        if enable_allure:
            allure_dir = self.reports_dir / "allure-results"
            # 清除旧的 Allure 结果数据（在 pytest 阶段开始前清除）
            if os.path.exists(allure_dir):
                import shutil
                log.info(f"清除旧的 Allure 结果数据: {allure_dir}")
                try:
        ...
        if enable_html:
            try:
                ...
                if pytest_report_file.exists():
                    ...
                    # pytest-html 报告格式: <span class="passed">33 Passed,</span>
                    # 模式1: 提取 passed 数量
                    passed_match = re.search(r'<span[^>]*class="passed"[^>]*>(\d+)', html_content)
                    if passed_match:
                        pytest_passed = int(passed_match.group(1))
                    
                    # 模式2: 提取 failed 数量
                    failed_match = re.search(r'<span[^>]*class="failed"[^>]*>(\d+)', html_content)
                    if failed_match:
                        pytest_failed = int(failed_match.group(1))
                    
                    # 模式3: 提取 error 数量
                    error_match = re.search(r'<span[^>]*class="error"[^>]*>(\d+)', html_content)
                    if error_match:
                        pytest_errors = int(error_match.group(1))
                    
                    # 模式4: 提取 skipped 数量
                    skipped_match = re.search(r'<span[^>]*class="skipped"[^>]*>(\d+)', html_content)
                    if skipped_match:
                        pytest_skipped = int(skipped_match.group(1))
                    
                    pytest_total = pytest_passed + pytest_failed + pytest_errors + pytest_skipped
                    
                    log.info(f"从 pytest HTML 报告提取结果: total={pytest_total}, passed={pytest_passed}, failed={pytest_failed}, errors={pytest_errors}, skipped={pytest_skipped}")
                else:
                    log.warning(f"pytest HTML 报告不存在: {pytest_report_file}")
            except Exception as e:
        ...
        log.info(f"Pytest 结果: total={pytest_total}, passed={pytest_passed}, failed={pytest_failed}, errors={pytest_errors}, skipped={pytest_skipped}")
        
        # 合并 pytest 结果
        merged_result['total'] += pytest_total
        merged_result['passed'] += pytest_passed
